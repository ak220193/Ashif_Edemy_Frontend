...
import { getAdvertisement, getUniversityData, getSingleData, getAllRandomDataList  } from "../store/slice/universitiesSlice";
import { resetIdCounter, Tab, Tabs, TabList, TabPanel } from 'react-tabs';
import Link from 'next/link';
import Slider from 'react-slick';
import 'slick-carousel/slick/slick.css';
import 'slick-carousel/slick/slick-theme.css';
const SingleUniversityDetails = () => {

    const router = useRouter();
    const { universityList, advertisementLists, singleUniversityData, randomRecordsList, loader } = useSelector((state)=>state.university);
    const [currentItem, setCurrentItem] = useState({});
    const [currentBanner, setBanner] = useState('');
    const [searchText, setSearchText] = useState('');
    const [relatedUniversity, setRelatedUniversity] = useState([]);
    ...
    useEffect(() => {    
        let currentId = localStorage.getItem('currentUniversityId');
    ...
    useEffect(() => {   
        if(singleUniversityData && singleUniversityData._id){
            setBanner(singleUniversityData.Image);
            setCurrentItem(singleUniversityData)
        }
    ...
    useEffect(() => {    
        setRelatedUniversity(randomRecordsList)
    },[randomRecordsList]); 
    ...
    return (
        <React.Fragment>
            <Navbar />
            <PageBanner 
                pageTitle= {(currentItem && currentItem.Univ_name) ? currentItem.Univ_name : "University"}
                homePageUrl="/" 
                homePageText="Home" 
                activePageText={(currentItem && currentItem.Univ_name) ? currentItem.Univ_name : "University"}
            />  
            ...
            <div className="courses-details-area pb-100">
                <div className="container">
                    <div className="row">
                        ...
                        <div className="col-lg-4 col-md-12">
                            <div className="courses-details-info" style={{ marginTop: '-75px'}}>
                                <div className="image">
                                    <img style={{ width: "100%" }} src={`${currentBanner}`} alt="image" />
                                </div>
                                ...
                                <ul className="info">
                                    ...
                                    <li>
                                        <div className="d-flex justify-content-between align-items-center">
                                            <span><i className="flaticon-web"></i> University District </span>
                                            {currentItem.District}
                                        </div>
                                    </li>
                                    <li>
                                        <div className="d-flex justify-content-between align-items-center">
                                            <span><i className="flaticon-lock"></i> University State </span>
                                            { currentItem.State }
                                        </div>
                                    </li>
                                </ul>
                    ...
                    <div className='row pt-100'>
                        ...
                        <Slider {...settings}>
                        {
                            relatedUniversity && relatedUniversity.length ? (
                                relatedUniversity.slice(0, visibleItems).map((list, listIndex) => (
                                    <div className="col-lg-3 col-md-4 cur-pointer" key={listIndex}>
                                        <div className='slider-box'>
                                            <div className='slider-header'>
                                                ...
                                                <div className='text-header cur-pointer'>
                                                    ...
                                                    <div className="column">
                                                        <h5 style={{fontWeight: 'bold', color: "red"}} onClick={() => { handleEventClick(list._id); }}>{list.Univ_name}</h5>
                                                        <h6 style={{color:'#959595'}} onClick={() => { handleEventClick(list._id); }}>{list.State}</h6>
                                                    </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className='slider-body cur-pointer'><br/>
                                                Contact Us: <br/><hr></hr>
                                                <h6 style={{color:'#959595'}}>Phone : {list.Phone}</h6><hr></hr>
                                ...
                                relatedUniversity.slice(0, visibleItems).map((list, listIndex) => (
                                  <div className="col-lg-3 col-md-4" key={listIndex}>
                                    ...
                                    <div className="org-name-all" style={{ position: 'relative', zIndex: 1, color: 'red', cursor: 'pointer' }} onClick={() => { handleEventClick(list._id); }}>
                                        <h6>{list.Univ_name}</h6>
                                        <h6>{list.Yrofestab}</h6>
                                        <h6>{list.Specialised}</h6>
                                    </div>
                                    </div>
                                  </div>
                                ))
                        ...
                        <div className="col-lg-12 col-md-12 pb-100">                                                 
                            ...
                            {   relatedUniversity && relatedUniversity.length && relatedUniversity.length > 8 && visibleItems == 12 &&
                                <button className='show-btn' onClick= {handleShowMore}>Show More</button>
                            }

                            {   relatedUniversity && relatedUniversity.length && relatedUniversity.length > 8 && visibleItems == 20 &&
...
export default SingleUniversityDetails;